...
export interface TouchableAreaProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** 클릭/터치 이벤트 핸들러 */
  onPress?: (event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => void;
...
export const TouchableArea = forwardRef<HTMLDivElement, TouchableAreaProps>(
  ({
    ...
    // 터치/클릭 이벤트 통합 처리
    const handlePress = useCallback((event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
      if (disabled) return;
      
      // 햅틱 피드백 (지원되는 경우)
      if ('vibrate' in navigator && enableFeedback) {
        const intensity = feedbackIntensity === 'subtle' ? 10 : 
                         feedbackIntensity === 'medium' ? 20 : 30;
