...
export function BasicResponsiveComponent() {
  ...
  return (
    <div className="p-4">
      ...
      {deviceType === 'mobile' && (
        <div className="bg-green-100 p-4 rounded">📱 모바일 전용 콘텐츠</div>
      )}

      {deviceType === 'tablet' && (
        <div className="bg-blue-100 p-4 rounded">📱 태블릿 전용 콘텐츠</div>
      )}

      {deviceType === 'desktop' && (
        <div className="bg-purple-100 p-4 rounded">🖥️ 데스크톱 전용 콘텐츠</div>
      )}
    </div>
  );
}
...
export function ViewportBasedComponent() {
  ...
  return (
    <div className="p-4">
      ...
      {width >= 1024 && (
        <div className="bg-green-100 p-4 rounded">🖥️ 큰 화면 (1024px 이상)</div>
      )}
    </div>
  );
}
...
export function BreakpointComponent() {
  ...
  return (
    <div className="p-4">
      ...
      <div className="bg-gray-100 p-4 rounded mb-4">
        <p>
          현재 브레이크포인트: <strong>{breakpoint.current}</strong>
        </p>
      </div>

      <div className="space-y-2">
        <div
          className={`p-2 rounded ${breakpoint.matches('sm') ? 'bg-green-100' : 'bg-gray-100'}`}
        >
          SM 이상: {breakpoint.matches('sm') ? '✅' : '❌'}
        </div>
        <div
          className={`p-2 rounded ${breakpoint.matches('md') ? 'bg-green-100' : 'bg-gray-100'}`}
        >
          MD 이상: {breakpoint.matches('md') ? '✅' : '❌'}
        </div>
        <div
          className={`p-2 rounded ${breakpoint.matches('lg') ? 'bg-green-100' : 'bg-gray-100'}`}
        >
    ...
    <div className="p-4">
      ...
      >
        ...
        <div className="bg-green-100 p-4 rounded flex-1">
          ...
          <p className="text-sm">방향에 따라 레이아웃이 변경됩니다</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4">
      ...
      <div className="bg-gray-100 p-4 rounded mb-4 text-sm">
        <p>
          디바이스: {deviceType} | 브레이크포인트: {breakpoint}
        </p>
        <p>
          터치: {hasTouch ? 'Yes' : 'No'} | 방향:{' '}
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto py-8">
        <header className="text-center mb-8">
          ...
          <p className="text-gray-600">일반적인 사용 패턴들을 확인해보세요</p>
        </header>
