...
export function MeetingDetailModal({
  ...
  return (
    <Dialog open={!!meeting} onOpenChange={onClose}>
      ...
      >
        {/* 헤더 - 고정 */}
        <DialogHeader className="flex-shrink-0 p-4 sm:p-6 pb-2 sm:pb-4 border-b border-border/30">
          ...
          <DialogDescription className="text-xs sm:text-sm text-muted-foreground">
            {meetingTypeLabel} • {formatDate(meeting.date)} •{' '}
            {meeting.time || '시간 미설정'}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none modal-scroll-area px-4 sm:px-6 py-2 sm:py-6 space-y-4 sm:space-y-6 min-h-0">
          {!isEditingMeeting ? (
            ...
            <>
              {/* 미팅 정보 카드 */}
              <div className="bg-card/40 backdrop-blur-sm border border-border/30 rounded-2xl p-4 sm:p-6 shadow-lg">
        ...
        {/* 푸터 - 고정 */}
        <DialogFooter className="flex-shrink-0 gap-2 sm:gap-3 p-4 sm:p-6 pt-2 sm:pt-4 border-t border-border/30 bg-background/95 backdrop-blur-sm">
          <div className="flex w-full gap-2 sm:gap-3">
            {!isEditingMeeting ? (
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto sm:justify-end ml-auto">
                <Button
      ...
      <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              이 미팅을 삭제하시겠습니까? 삭제된 미팅은 복구할 수 없습니다.
              {(meeting as any)?.syncToGoogle && (
