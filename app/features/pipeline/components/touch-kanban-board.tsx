...
export function TouchKanbanBoard({
  ...
  const [verticalScrollPositions, setVerticalScrollPositions] = useState<
    Record<string, number>
  >({});
  const [visibleCardRanges, setVisibleCardRanges] = useState<
  ...
  const filteredStages = useMemo(() => {
    if (stageFilterState.viewMode === 'all') {
      ...
      stageFilterState.viewMode === 'single' &&
      stageFilterState.currentSingleStage
    ) {
      return stages.filter(
        stage => stage.id === stageFilterState.currentSingleStage
      );
    } else {
      return stages.filter(stage =>
        stageFilterState.selectedStages.includes(stage.id)
      );
    }
  ...
  // 단계별 클라이언트 그룹핑
  const clientsByStage = clientsState.reduce(
    (acc, client) => {
  ...
  const handleTouchStart = useCallback(
    (e: React.TouchEvent, clientId: string, stageId: string) => {
      e.preventDefault();
  ...
  const handleTouchMove = useCallback(
    ...
    [
      ...
      touchState.startX,
      touchState.startY,
      touchState.sourceStageId,
      touchState.targetStageId,
      touchState.dragIntensity,
    ]
  );
  ...
  const handleTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      ...
      const { draggedElement, draggedClientId, sourceStageId, targetStageId } =
        touchState;
  ...
  const handleVerticalScroll = useCallback(
    (stageId: string, scrollTop: number) => {
      ...
      const visibleStart = Math.max(
        0,
        Math.floor(scrollTop / CARD_HEIGHT) - VISIBLE_CARDS_BUFFER
      );
      const visibleEnd = Math.min(
        clientsByStage[stageId]?.length || 0,
        Math.ceil((scrollTop + CONTAINER_HEIGHT) / CARD_HEIGHT) +
          VISIBLE_CARDS_BUFFER
      );
  ...
  const getVisibleCards = useCallback(
    (stageId: string, stageClients: Client[]) => {
      const range = visibleCardRanges[stageId];
      if (!range || stageClients.length <= 10) {
        return {
          visibleCards: stageClients,
          totalHeight: stageClients.length * CARD_HEIGHT,
        };
      }
      ...
      return {
        visibleCards,
        totalHeight,
        offsetTop: range.start * CARD_HEIGHT,
      };
    },
    ...
    <div className="w-full">
              ...
              <div className="space-y-1 text-xs text-muted-foreground">
                <div>
                  ...
                  {touchState.dragDirection === 'horizontal'
                    ? '좌우'
                    : touchState.dragDirection === 'vertical'
                      ? '상하'
                      : '자유'}{' '}
                  드래그
                </div>
              ...
              return (
                <div
                  ...
                  className={`min-w-[300px] transition-all duration-200 ${
                    isTargetStage
                      ? 'transform scale-[1.02] ring-2 ring-primary'
                      : ''
                  }`}
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {filteredStages.map(stage => {
              ...
              const {
                ...
                totalHeight,
                offsetTop = 0,
              } = getVisibleCards(stage.id, stageClients);
              ...
              return (
                ...
                >
                  ...
                  >
                    {isCollapsed ? (
                      <div
                        className={`flex flex-col items-center justify-center h-32 bg-muted/20 border border-dashed border-border rounded-lg ${
                          touchState.isDragging &&
                          isTargetStage &&
                          touchState.isValidDropZone
                      ...
                      >
                        {stageClients.length > 0 ? (
                          <>
                                ...
                                {visibleCards.map(client => (
                                  <div key={client.id} className="relative">
                                    ...
                                    {touchState.isDragging &&
                                      touchState.draggedClientId &&
                                      stage.id ===
                                        touchState.dragPlaceholderStage &&
                                      stage.id !== touchState.sourceStageId && (
                            ...
                            {stageClients.length > 10 && (
                              <div className="absolute top-2 right-2 bg-primary/10 text-primary text-xs px-2 py-1 rounded-full">
                                {Math.floor(
                                  (verticalScrollPositions[stage.id] || 0) /
                                    CARD_HEIGHT
                                ) + 1}{' '}
                                / {stageClients.length}
                              </div>
                          ...
                          <div
                            className={`flex flex-col items-center justify-center h-32 bg-muted/10 border border-dashed border-border rounded-lg ${
                              touchState.isDragging &&
                              isTargetStage &&
                              touchState.isValidDropZone
                                ? 'border-primary bg-primary/5 scale-105'
                          ...
                          >
                            ...
                            <p className="text-xs text-muted-foreground">
                              {touchState.isDragging && isTargetStage
                                ? ''
                                : '고객을 추가하거나 드래그하세요'}
                            </p>
                          </div>
      ...
      {quickEditMode && selectedClientId && (
        <QuickEditModal
          ...
          client={
            clientsState.find(client => client.id === selectedClientId) || null
          }
