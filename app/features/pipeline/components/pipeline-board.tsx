  ...
  return (
    ...
    >
      ...
      <div className="lg:hidden bg-background">
        {/* 🎯 Sticky 헤더 네비게이션 (고객 상세와 동일한 패턴) */}
        <div
          className={`sticky top-0 z-40 bg-background border-b border-border transition-all duration-300 ${isScrolled ? 'py-2' : 'py-4'}`}
          style={{
            // 🎯 성능 최적화: 스크롤 시 하드웨어 가속
            transform: 'translateZ(0)',
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center space-x-3 min-w-0 flex-1">
                          <div
                            className={`rounded-full flex-shrink-0 transition-all duration-200 ${isScrolled ? 'w-2 h-2' : 'w-3 h-3'}`}
                            style={{
                              backgroundColor: stage.color,
                              // 🎯 성능 최적화: 색상 변경은 paint 트리거하지 않음
                              transform: 'translateZ(0)',
                            }}
                          />
                          <h3
                            className={`font-semibold text-foreground truncate transition-all duration-200 ${isScrolled ? 'text-sm' : 'text-base'}`}
                          >
                            {stage.name}
                          </h3>
                        </div>
            ...
            <CarouselContent className="-ml-1">
              {stages.map(stage => {
                ...
                return (
                  ...
                  >
                    <div
                      className="w-full px-4 min-h-screen"
                      data-drop-zone="true"
                            ...
                            <div className="space-y-3">
                              {stageClients.map(client => (
                                <div
                                  ...
                                  onTouchStart={e =>
                                    handleTouchStart(e, client.id, stage.id)
                                  }
                                  onTouchMove={handleTouchMove}
                                  onTouchEnd={handleTouchEnd}
                                  className={`cursor-grab active:cursor-grabbing touch-manipulation select-none ${
