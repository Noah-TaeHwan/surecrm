 ...
 * - 행 단위 가상화로 성능 최적화
 */
export const VirtualizedClientList = memo<VirtualizedClientListProps>(function VirtualizedClientList({
  clients,
  onClientClick,
  onAddClient,
  isLoading = false,
  ...
  const renderLoadingState = () => (
    <div className="space-y-4 sm:space-y-6 p-4 sm:p-6">
      {Array.from({ length: 4 }).map((_, rowIndex) => (
        <div key={rowIndex} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
  ...
  const scrollToClient = (clientId: string) => {
    const clientIndex = filteredClients.findIndex((client) => client.id === clientId);
    if (clientIndex !== -1) {
            ...
            <VList
              ...
              onScroll={(offset) => {
                ...
                if (process.env.NODE_ENV === 'development') {
                  console.log('Virtual scroll offset:', offset, 'rows:', clientRows.length);
                }
              }}
            >
              {/* 🎯 가상화된 행들 렌더링 */}
