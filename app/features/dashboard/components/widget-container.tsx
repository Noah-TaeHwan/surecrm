  ...
  const getPriorityIndicator = () => {
    ...
    return (
      <div className="absolute top-2 right-2 z-10">
        <div className={cn('w-2 h-2 rounded-full', colors[widget.priority])} />
      </div>
    );
  };
...
export function useWidgetContainer(initialWidgets: WidgetConfig[]) {
  ...
  const setWidgetLoading = useCallback((widgetId: string, loading: boolean) => {
    setWidgets(prev =>
      prev.map(widget =>
        widget.id === widgetId ? { ...widget, loading } : widget
      )
    );
  }, []);
  ...
  const setWidgetError = useCallback(
    (widgetId: string, error: string | null) => {
      setWidgets(prev =>
        prev.map(widget =>
          widget.id === widgetId ? { ...widget, error } : widget
        )
      );
    },
    []
